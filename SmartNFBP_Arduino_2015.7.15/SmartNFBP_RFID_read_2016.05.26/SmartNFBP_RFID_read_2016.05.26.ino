...
void setup(void)
{
  ...
  if (! versiondata) {							//如果版本返回0，则死循环
    ...
    while (1)
    {
      ...
      digitalWrite(LEDPin, LOW);	//Shut DOWN LED
      delay(100);
      digitalWrite(Buzzer, HIGH);	//蜂鸣器响
      digitalWrite(LEDPin, HIGH);	//打开 LED
      delay(300);					//延时
      ...
      if (sta && buf[0] == 4) {
        ...
        if (sta) {
          ...
          if (sta) {
            ...
            nfc.puthex(block, 1);//打印IC卡里第一个区域里面 1个字符
            Serial.println();
          }
  ...
  if (sta && buf[0] == 4) {
    ...
    if (sta) {
      ...
      if (sta) {
        ...
        RFID_String_new=nfc.puthex(block, 16);	//打印IC卡里第一个区域里面 1个字符       
        //////////////////////////////////////////////////////////////////////////////
        //////////////////////////////////////////////////////////////////////////////
        //Serial.println();		//换行
        ...
        {
          Serial.println(RFID_String_new);
          RFID_String_old=RFID_String_new;	//把老的替换掉
        //////////  Beep ///////////////////
        digitalWrite(Buzzer, HIGH);	//蜂鸣器响
        delay(200);					//延时
        digitalWrite(Buzzer, LOW);
...
void distance()
{
  ...
  //此函数的计时功能由经验决定，长时间的脉冲计时可能会出错。计时范围从10微秒至3分钟。（1秒=1000毫秒=1000000微秒）
  //接收到的高电平的时间（us）* 340m/s / 2 = 接收到高电平的时间（us） * 17000 cm / 1000000 us = 接收到高电平的时间 * 17 / 1000  (cm)
  if (Value_cm < 100 && Value_cm >10)
  {
    Serial.print("V!_");
    Serial.println(Value_cm);
    //写入智能测距及智能报警，越远频率越低，越近频率越高。
    int Time_Beep=0;
    int Time_T=0;
