...
/** data buffer */
u8 nfc_buf[NFC_CMD_BUF_LEN];	//杩欎釜鏄懡浠や紶閫掔殑鏁版嵁甯� 闀垮害涓�64
 ...
 */
/*****************************************************************************/
void NFC_Module::begin(void)	//鍒濆鍖�
{
#ifdef PN532DEBUG
  u8 i;
#endif
...
#ifdef PN532DEBUG
  for(i=0; i<16; i++){
    Serial.write((u8)hextab[i]);	//娴嬭瘯 鍐欏叆 01234567890ABCDEF
  }
  for(i=0; i<6; i++){
    Serial.write((u8)ack[i]);		//娴嬭瘯淇″彿
  }
  for(i=0; i<6; i++){
    Serial.write((u8)nfc_version[i]);//鐗堟湰
  }
#endif
}
...
{
  u32 version;//32浣� 鐨勭増鏈彿

  nfc_buf[0] = PN532_COMMAND_GETFIRMWAREVERSION;//鍐欏叆鍛戒护
...
/*****************************************************************************/
/*!
 	@brief card inventory.	锟叫断匡拷片锟角凤拷锟斤拷锟�
...
/*****************************************************************************/
/*!
 	@brief  Mifare funciton. Read a block.  璇诲彇鍒嗗尯
...
u8 NFC_Module::MifareReadBlock(u8 block, u8 *buf)
{
  ...
  memcpy(buf, nfc_buf+8, 16);	//澶嶅埗鍒� buf
  /* 
...
/*****************************************************************************/
/*!
 	@brief  Send hexadecimal data through Serial with specified length.	锟斤拷锟�16锟斤拷锟斤拷
 	@param  buf - pointer of data buffer. 鏁版嵁鍖哄煙
 	@param  len - length need to send. 闇�瑕佸彂閫佺殑闀垮害
 	@return NONE
 */
/*****************************************************************************/
String NFC_Module::puthex(u8 *buf, u32 len)	// 杩欎釜鏄鍙� 16杩涘埗鐨勯儴鍒�
{
  u32 i;
  int characterleo=0;
  String back_string="";		// 寤虹珛涓�涓殏瀛樺瓧绗︿覆锛屼繚瀛樻敹鍒扮殑瀛楃锛屽苟鎶婂畠鏁翠綋杩斿洖
  for(i=0; i<len; i++)
  {
    //16
    //Serial.write(hextab[(buf[i]>>4)&0x0F]);		//楂樺瓧绗﹀啓鍏�
    //0x0F 2杩涘埗涓� 00001111 
    // hextab[17]="0123456789ABCDEF";
    if((int)(hextab[(buf[i]>>4)&0x0F])<58)
    {
      characterleo=((int)(hextab[(buf[i]>>4)&0x0F])-48)*16;
    }
    else
    {
      characterleo=((int)(hextab[(buf[i]>>4)&0x0F])-55)*16;
    }
    //Serial.write('0'-48+characterleo);
    //Serial.write(hextab[buf[i]&0x0F]);			//浣庡瓧绗﹀啓鍏�
    if(((int)(hextab[buf[i]&0x0F]))<58)
    {
      characterleo+=((int)(hextab[buf[i]&0x0F])-48);
    }
    else
    {
      characterleo+=((int)(hextab[buf[i]&0x0F])-55);
    }
    //characterleo=((int)(hextab[buf[i]&0x0F]));//+characterleo;
    //Serial.write('0'-48+characterleo);
    //Serial.write('*');
    //Serial.write('0'-48+characterleo);
    back_string+=char('0'-48+characterleo); 	//缁勫悎瀛楃鎴愬瓧绗︿覆
    characterleo=0;
    //Serial.write(' ');							//鍒嗗壊绗�
  }
  return back_string;
...
void NFC_Module::read_dt(u8 *buf, u8 len)
{
  ...
  {
    delay(1);
    buf[i] = receive();	//I2C 闀垮害鏄�26 (涓�鑸儏鍐�)
