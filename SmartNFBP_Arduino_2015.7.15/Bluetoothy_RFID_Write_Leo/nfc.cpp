...
/** data buffer */
u8 nfc_buf[NFC_CMD_BUF_LEN];	//这个是命令传递的数据帧 长度为64
 ...
 */
/*****************************************************************************/
void NFC_Module::begin(void)	//初始化
{
#ifdef PN532DEBUG
  u8 i;
#endif
...
#ifdef PN532DEBUG
  for(i=0; i<16; i++){
    Serial.write((u8)hextab[i]);	//测试 写入 01234567890ABCDEF
  }
  for(i=0; i<6; i++){
    Serial.write((u8)ack[i]);		//测试信号
  }
  for(i=0; i<6; i++){
    Serial.write((u8)nfc_version[i]);//版本
  }
#endif
}
...
{
  u32 version;//32位 的版本号

  nfc_buf[0] = PN532_COMMAND_GETFIRMWAREVERSION;//写入命令
...
/*****************************************************************************/
/*!
 	@brief  Mifare funciton. Read a block.  读取分区
...
u8 NFC_Module::MifareReadBlock(u8 block, u8 *buf)
{
  ...
  memcpy(buf, nfc_buf+8, 16);	//复制到 buf
  /* 
...
/*****************************************************************************/
/*!
 	@brief  Send hexadecimal data through Serial with specified length.	���16����
 	@param  buf - pointer of data buffer.
 	@param  len - length need to send.
 	@return NONE
 */
/*****************************************************************************/
void NFC_Module::puthex(u8 *buf, u32 len)	//����ַ�
{
  u32 i;
  int characterleo=0;
  for(i=0; i<len; i++)
  {
    //16
    //Serial.write(hextab[(buf[i]>>4)&0x0F]);		//高字符写入
    //0x0F 2进制为 00001111 
    ...
    //Serial.write(hextab[buf[i]&0x0F]);			//低字符写入
